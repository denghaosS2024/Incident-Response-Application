...
class PersonnelController {
    ...
    async getAllAvailablePersonnel() {
        try {
            ...
            return unassignedUsers.map(
                ({ _id, username, assignedCity, role }) => ({
                    _id,
                    name: username,
                    assignedCity,
                    role,
    ...
    async updatePersonnelCity(
        ...
        try {
            ...
            if (
                ...
                throw new Error(
                    `Personnel with username '${username}' does not exist`,
                )
            }
            ...
            if (!cityExists) {
                throw new Error(
                    `City '${cityName}' does not exist in the database`,
                )
            }
            const updatedPersonnel = await User.findOneAndUpdate(
                { username },
    ...
    async releaseVehicleFromPersonnel(
        ...
        if (personnel.role === ROLES.POLICE) {
            ...
            if (!truck) {
                throw new Error(
                    `Truck with name '${vehicleName}' does not exist`,
                )
            }
            const updatedPersonnel = await User.findOneAndUpdate(
                { username: personnelName },
