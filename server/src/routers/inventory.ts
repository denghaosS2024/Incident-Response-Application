...
export default Router()
  ...
  .get("/non-default", async (_req, res) => {
    try {
      ...
      const nonDefaultInventories =
        await InventoryController.getAllNonDefaultInventories();
  ...
  .get("/default/item/:itemName", async (req, res) => {
    try {
      ...
      const defaultInventory =
        await InventoryController.getInventoryByCategory("default");
      ...
      const item = defaultInventory.items.find(
  ...
  .put("/default/item", async (req, res) => {
    const { name, quantity, description, icon } = req.body;
    ...
    try {
      const updatedInventory =
        await InventoryController.addOrUpdateItemInDefaultCategory({
          name,
          quantity,
          description,
          icon,
  ...
  .delete("/deleteitem", async (req, res) => {
    ...
    try {
      const result =
        await InventoryController.deleteItemFromAllCategories(name);
      res.status(200).json(result);
    } catch (error) {
      console.error("Error deleting item from all inventories:", error);
      res.status(500).json({
        error: error instanceof Error ? error.message : "Unknown error",
      });
    }
