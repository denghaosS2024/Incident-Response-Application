...
describe("TruckController", () => {
  ...
  it("should remove username and deallocate truck from incident if no usernames remain", async () => {
    ...
    expect(
      incidentAfter?.assignedVehicles.find((v) => v.name === "Truck1"),
    ).toBeUndefined();
  });
  ...
  describe("", () => {
    it("should return trucks that are not assigned to incidents and have responders", async () => {
      ...
      const availableTrucks =
        await TruckController.getAvailableTrucksWithResponder();
      ...
      const truckNames = availableTrucks
        .map((truck) => truck.name)
        .sort((a, b) => a.localeCompare(b));
      expect(truckNames).toEqual(
        ["Truck 1", "Truck 2"].sort((a, b) => a.localeCompare(b)),
      );
      ...
      expect(
        availableTrucks[0].name.localeCompare(availableTrucks[1].name),
    ...
    it("should release a username and update both Truck and Incident if needed", async () => {
      ...
      const result = await TruckController.releaseUsernameFromTruck(
        "Truck-Integration",
        "usr",
      );
    ...
    it("should throw an error when database operation fails", async () => {
      ...
      // Assertions
      await expect(
        TruckController.getAvailableTrucksWithResponder(),
  ...
  describe("TruckController.addUsernameToTruck", () => {
    ...
    it("should throw an error when the truck does not exist", async () => {
      ...
      await expect(
        TruckController.addUsernameToTruck(
          nonExistenttruckName,
          username,
          null,
        ),
      ).rejects.toThrow(
        `Truck with name '${nonExistenttruckName}' does not exist`,
      );
    });
