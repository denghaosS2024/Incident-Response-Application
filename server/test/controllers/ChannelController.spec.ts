...
describe('Channel controller', () => {
    ...
    beforeAll(async () => {
        ...
        userA = await UserController.register(
            'Channel-User-A',
            'password-A',
            ROLES.CITIZEN,
        )
        userB = await UserController.register(
            'Channel-User-B',
            'password-B',
            ROLES.CITIZEN,
        )
        userC = await UserController.register(
            'Channel-User-C',
            'password-C',
            ROLES.CITIZEN,
        )
    ...
    it('can list all existing channels', async () => {
        ...
        // Concurrency issue with DB insertion order
        const names = new Set(result.map((ch) => ch.name))
    ...
    it('should return uploadUrl and fileUrl for an existing channel for video upload', async () => {
        ...
        const { uploadUrl, fileUrl } =
            await ChannelController.getVideoUploadUrl(testChannel._id)
    ...
    it('should return uploadUrl and fileUrl for an existing channel for image upload', async () => {
        ...
        const { uploadUrl, fileUrl } =
            await ChannelController.getImageUploadUrl(testChannel._id)
    ...
    it('should return voiceMessageUrl for an existing channel for sending voice message', async () => {
        ...
        // Call getFileUploadUrl
        const { uploadUrl, fileUrl } =
            await ChannelController.getVoiceUploadUrl(
                testChannel._id,
                'recording',
            )
    ...
    it('should handle error if GCS getSignedUrl call fails for video upload', async () => {
        ...
        const result = await ChannelController.getVideoUploadUrl(
            testChannel._id,
        )
    ...
    it('should handle error if GCS getSignedUrl call fails for image upload', async () => {
        ...
        const result = await ChannelController.getImageUploadUrl(
            testChannel._id,
        )
        expect(result).toEqual({ error: 'Error generating signed URL' })
    })
    ...
    it('can acknowledge a message and notify other users', async () => {
        ...
        expect(acknowledged.acknowledgedBy[0]._id.toHexString()).toBe(
            userA._id.toHexString(),
        )
    ...
    it('should make a phone call between two users and notify the other user', async () => {
        ...
        expect(result.message.content).toBe(
            `Phone call started now between ${userA.username} and ${userB.username}.`,
        )
    ...
    it('can get closed groups', async () => {
        ...
        const hasClosedChannel = closedGroups.some(
            (ch) => ch.name === 'A Closed Channel',
        )
        expect(hasClosedChannel).toBe(true)

        // Verify open channel is not included
        const hasOpenChannel = closedGroups.some(
            (ch) => ch.name === 'Open Channel',
        )
    ...
    it('can get closed groups sorted by name', async () => {
        ...
        const hasChannel1 = closedGroups.some(
            (ch) => ch.name === 'Z Closed Channel',
        )
        const hasChannel2 = closedGroups.some(
            (ch) => ch.name === 'A Closed Channel',
        )
        ...
        const channel1Index = closedGroups.findIndex(
            (ch) => ch.name === 'Z Closed Channel',
        )
        const channel2Index = closedGroups.findIndex(
            (ch) => ch.name === 'A Closed Channel',
        )
    ...
    it('can add a user to an existing channel', async () => {
        ...
        expect(updatedChannel.users.some((u) => u._id.equals(userC._id))).toBe(
            true,
        )
    })
    ...
    it('can remove a user from an existing channel', async () => {
        ...
        const updatedChannel = await ChannelController.updateChannel({
            ...
            userIds: channel.users
                .filter((u) => !u._id.equals(userB._id))
                .map((u) => u._id), // Remove userB
        })

        // Ensure the channel has the correct number of users
        expect(updatedChannel.users.length).toBe(1)
        expect(updatedChannel.users.some((u) => u._id.equals(userB._id))).toBe(
            false,
        )
    })
    ...
    it('should start a video conference and notify only other online users', async () => {
        ...
        expect(message.content).toMatch(
            /^Video conference started! Join here: https:\/\/meet\.jit\.si\/.+/,
        )
        expect(message.sender._id).toEqual(userA._id)
