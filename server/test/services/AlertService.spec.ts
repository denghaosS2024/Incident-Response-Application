...
import AlertService, {
  AlertService as AlertServiceClass,
...
describe("AlertService utility functions", () => {
  ...
  describe("alertComparator", () => {
    it("should prioritize by alert type first", () => {
      ...
      expect(
        AlertServiceClass["alertComparator"](
          lowPriorityAlert,
          highPriorityAlert,
        ),
      ).toBeGreaterThan(0);
    ...
    it("should use creation time as tiebreaker for same priority", () => {
      ...
      expect(
        AlertServiceClass["alertComparator"](olderAlert, newerAlert),
      ).toBeLessThan(0);
    });
...
describe("promoteNextAlert", () => {
  it("should send the next alert from the queue", async () => {
    ...
    const sendAlertSpy = jest
      .spyOn(AlertService as any, "sendAlertNow")
