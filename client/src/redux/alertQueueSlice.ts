...
export const alertQueueSlice = createSlice({
  ...
  reducers: {
    addAlert: (state, action: PayloadAction<IMessage>) => {
      ...
      console.log("Redux: addAlert called with:", {
        id: alert._id,
        channelId,
        content,
      });
      ...
      if (priority > currentPriority) {
        console.log(
          "New alert has higher priority, replacing active:",
          alert._id,
        );
        ...
        state.queues[channelId].sort((a, b) => {
          const priorityA = getPriorityLevel(
            getAlertTypeFromContent(a.content),
          );
          const priorityB = getPriorityLevel(
            getAlertTypeFromContent(b.content),
          );
        ...
        console.log(
          "QUEUE UPDATED - Current queue for channel",
          channelId,
          ":",
          state.queues[channelId].map((a) => ({
            id: a._id,
            content: a.content,
    ...
    completeActiveAlert: (
      state,
      action: PayloadAction<{ channelId: string; alertId: string }>,
    ) => {
      const { channelId, alertId } = action.payload;
      console.log("Redux: completeActiveAlert called:", { channelId, alertId });
      console.log(
        "Redux: current active alert:",
        state.activeAlerts[channelId]?._id,
      );
      ...
      if (state.activeAlerts[channelId]?._id === alertId) {
        ...
        if (state.queues[channelId]?.length > 0) {
          ...
          if (nextAlert) {
            ...
            console.log(
              "QUEUE UPDATED - Current queue after next alert activated for channel",
              channelId,
              ":",
              state.queues[channelId].map((a) => ({
                id: a._id,
        ...
        if (state.queues[channelId]) {
          const queueIndex = state.queues[channelId].findIndex(
          ...
          if (queueIndex >= 0) {
            ...
            console.log(
              "QUEUE UPDATED - Current queue after alert removed for channel",
              channelId,
              ":",
              state.queues[channelId].map((a) => ({
                id: a._id,
...
export const { addAlert, completeActiveAlert, clearQueue, resetAlertQueue } =
  alertQueueSlice.actions;

// Selectors
export const selectActiveAlert = (
  state: { alertQueue: AlertQueueState },
  channelId: string,
) => state.alertQueue.activeAlerts[channelId];

export const selectQueue = (
  state: { alertQueue: AlertQueueState },
  channelId: string,
) => state.alertQueue.queues[channelId];

export const selectHasActiveAlert = (
  state: { alertQueue: AlertQueueState },
  channelId: string,
) => !!state.alertQueue.activeAlerts[channelId];
