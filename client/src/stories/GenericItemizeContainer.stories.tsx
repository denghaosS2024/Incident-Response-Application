...
export const WithTitleAndListItems: StoryObj<
  GenericItemizeContainerProps<User>
> = {
  args: {
    title: "User List",
    items: users,
...
export const WithHeaderLabelsAndCustomRendering: StoryObj<
  GenericItemizeContainerProps<User>
> = {
  args: {
    items: users,
    ...
    columns: [
      ...
      },
      {
        ...
        label: "Contact Email",
        render: (item) => (
          <Link href={`mailto:${item.email}`} underline="hover" color="primary">
            {item.email}
          </Link>
        ),
        align: "center",
      },
      {
        key: "role",
        label: "User Role",
        render: (item) => <Typography variant="body2">{item.role}</Typography>,
        align: "center",
      },
    ],
  },
