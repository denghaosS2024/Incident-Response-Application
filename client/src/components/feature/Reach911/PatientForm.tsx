  ...
  const handleProfileClick = async () => {
    if (patientUsername) {
      try {
        ...
        alert("Failed to retrieve user information. Please try again later.");
      }
    } else {
      ...
      if (patientUsername) {
        try {
          ...
          if (response && response.userId) {
            ...
            console.log("User ID fetched from database:", response.userId);
          } else {
            console.error("User not found");
            alert("User not found in the database.");
          }
        } catch (error) {
          console.error("Failed to fetch user ID from the database:", error);
          alert("Failed to retrieve user information. Please try again later.");
        }
      }
    };
    ...
    request("/api/users/createTemp", { method: "POST" })
      .then((data) => {
        if (data.userId && data.username) {
          ...
          alert("Failed to retrieve new userId or username from server.");
        }
      })
      .catch((error) => {
