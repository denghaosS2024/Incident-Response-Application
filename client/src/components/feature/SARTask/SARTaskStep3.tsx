...
const SARTaskStep3: React.FC<SARTaskStep3Props> = ({
  incident,
  setIncident,
}) => {
  ...
  useEffect(() => {
    ...
    const victims =
      incident?.sarTasks?.at(taskId)?.victims || categories.map(() => 0);
    setCounts(victims);
  ...
  const handleCountChange = (index: number, value: string) => {
    ...
    if (newCounts.length === categories.length) {
      setAllowTreatVictim(
        newCounts[0] > 0 || newCounts[1] > 0 || newCounts[2] > 0,
      );
    }
  };
  ...
  const saveData = async () => {
    ...
    if (!needSave) {
      console.log(
        "[Step3] Form data not updated. Do not update to database",
        new Date(),
      );
      return;
    }
