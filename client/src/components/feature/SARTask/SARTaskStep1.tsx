...
const SARTaskStep1: React.FC<SARTaskStep1Props> = ({
  incident,
  setIncident,
}) => {
  ...
  const startDate = incident?.sarTasks?.at(taskId)?.startDate
    ? new Date(incident?.sarTasks?.at(taskId)?.startDate || "")
    : new Date();
  ...
  useEffect(() => {
    ...
    if (
      ...
      incident?.sarTasks?.at(taskId)?.state === "Todo"
    ) {
      updateSARTask().then();
    }
  }, [incident]);
  ...
  return (
    <div className={styles.wrapperStep}>
      ...
      <div className={styles.flexCenter}>
        <FEMAMarker left={leftText} size={300} />
      </div>
      <div
