...
const SARTaskStep2: React.FC<SARTaskStep2Props> = ({
  incident,
  setIncident,
}) => {
  ...
  const saveData = async () => {
    ...
    if (!needSave) {
      console.log(
        "[Step2] Form data not updated. Do not update to database",
        new Date(),
      );
      return;
    }
    ...
    const hazards = Object.keys(selectedHazards).filter(
      (hazard) => selectedHazards[hazard] && hazard !== "Others",
    );
