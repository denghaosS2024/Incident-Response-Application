...
const SARStep4: React.FC<TaskStatsProps> = () => {
    ...
    try {
      ...
      if (incidentDoc && incidentDoc[0]?.sarTasks) {
        ...
        const todoTasks = tasks.filter(
          (task: ISarTask) => task.state === "Todo",
        );
        const inProgressTasks = tasks.filter(
          (task: ISarTask) => task.state === "InProgress",
        );
        ...
        doneTasks.forEach((task: ISarTask) => {
          if (task.startDate) {
            ...
            const closingDate = task.endDate
              ? new Date(task.endDate)
              : new Date(); // Use current date if missing
            totalHoursSpent +=
  ...
  const fetchVictimStats = async () => {
    try {
      ...
      if (
        !incidentDoc ||
        incidentDoc.length === 0 ||
        !incidentDoc[0].sarTasks
      ) {
      ...
      sarTasks.forEach((task: ISarTask) => {
        if (Array.isArray(task.victims) && task.victims.length === 5) {
          ...
          totalVictims += task.victims.reduce(
  ...
  const fetchIncidentById = async () => {
    ...
    try {
      ...
      const incidentData = Array.isArray(incidentDoc)
        ? incidentDoc[0]
        : incidentDoc;
  ...
  return (
    <Box sx={{ textAlign: "center", mt: 4, margin: "auto" }}>
      ...
      <Box sx={{ mt: 4, border: "solid" }}>
        ...
        <Typography>
          Active Electric Wire: {hazardStats["Active Electric Wire"]}
        </Typography>
