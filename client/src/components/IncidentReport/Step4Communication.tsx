...
const Step4Communication: React.FC<Step4CommunicationProps> = ({
  ...
  useEffect(() => {
    const fetchMessagesWithSenders = async () => {
      if (!incidentData.incidentCallGroup && !incidentData.respondersGroup) {
        return;
      }
      ...
      try {
          ...
          try {
            ...
            for (const message of callerMessages) {
              try {
                ...
                console.warn(`Error processing message data:`, error);
                allMessages.push({
                  _id: message._id,
                  content: message.content,
                  sender:
            ...
            console.error("Error fetching caller messages:", fetchError);
        ...
        if (incidentData.respondersGroup) {
          ...
          try {
            ...
            const responderMessages = Array.isArray(respondersResponse)
              ? respondersResponse
              : [];
            ...
            for (const message of responderMessages) {
              try {
                ...
                console.warn(`Error processing responder message data:`, error);
                allMessages.push({
                  _id: message._id,
                  content: message.content,
                  sender:
            ...
            console.error("Error fetching responder messages:", fetchError);
  ...
  const getMessageIcon = (message: Message) => {
    ...
    if (role.includes("fire") || name.toLowerCase().includes("fire")) {
      ...
      return <span style={{ fontWeight: "bold", color: "#d32f2f" }}>911</span>;
    }
  ...
  const parseAlertContent = (message: Message) => {
    if (!message.isAlert) return { text: "", bgColor: null, textColor: null };
