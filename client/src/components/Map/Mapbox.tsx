...
const Mapbox: React.FC<MapboxProps> = ({
    ...
    try {
      ...
      for (const incident of incidents) {
        ...
        try {
          ...
          console.error(
            `Error geocoding incident address: ${incident.address}`,
            error,
          );
        }
      }
  ...
  const fetchAndDisplayTrucks = async () => {
    try {
      ...
      const userWithTruck = allUsers.find(
        (user: any) => user.assignedTruck !== null,
      );
  ...
  const fetchAndDisplayCars = async () => {
    try {
      ...
      const userWithCar = allUsers.find(
        (user: any) => user.assignedCar !== null,
      );
...
`;
  ...
  const updateSARTaskMarkers = (data: {
    tasks: any[];
    incidentId?: string;
  }) => {
    if (!mapRef.current) return;
  ...
  const handleAddSARTask = async (type: string) => {
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        const finalAddress = await getAddressFromCoordinates(
          finalLngLat.lng,
          finalLngLat.lat,
        );
