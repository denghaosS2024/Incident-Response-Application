...
const Column: React.FC<ColumnProps> = ({ title, subtitle, tasks, id, onGroupClick, canDrag, onGroupDrop }) => {
        ...
        request('/api/channels', {
            ...
            .then((data) => {
                data = data.filter((channel: IChannel) => {
                    return channel.users.some(user => user._id === owner && channel.name !== "PrivateContact");
                });
                setGroups(data);
            })
                        ...
                        {id === "1" && (
                            <>
                                ...
                                {groups.map((group, index) => (
                                    <Draggable key={group._id} draggableId={`group-${group._id}`} index={index}>
                                        {(provided) => (
                                            <GroupCard
                                                ref={provided.innerRef}
                                                {...provided.draggableProps}
                                                {...provided.dragHandleProps}
                                                onClick={() => onGroupClick ? onGroupClick(group._id) : undefined}
                                            >
                                                {group.name}
                                            </GroupCard>
                                        )}
